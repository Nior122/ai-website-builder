...
const STRATEGIES: RepairStrategy[] = [
  ...
  { name: 'convert-undefined-null-strings', apply: (text) => { let c = text.replace(/:\s*undefined(?=\s*[,}\]])/g,': null').replace(/:\s*None(?=\s*[,}\]])/g,': null').replace(/:\s*"undefined"(?=\s*[,}\]])/g,': null'); return c !== text ? c : null; } },
