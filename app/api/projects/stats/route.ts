...
export const GET = withRequestLogging(
  withRateLimit(async (request: NextRequest) => {
    try {
      const { userId } = await auth(); if (!userId) return unauthorized();
      const dbUserId = await resolveDbUserId(userId);
      const { limits } = await checkSubscription(dbUserId);
      const [totalProjects, publishedProjects, aiGenerationsUsed, exportsUsed] = await Promise.all([
      ...
      return ok({ totalProjects, publishedProjects, aiGenerationsUsed, aiGenerationsLimit: limits.aiGenerations, exportsUsed, exportsLimit: limits.exports });
    } catch (err) { return errorResponse(err instanceof Error ? err : new Error(String(err))); }
  }, { tier: 'free' })
);
